...
public class EntitatLogicaEJB extends EntitatEJB implements EntitatLogicaService, EntitatFields {
    ...
    public void deleteFull(Entitat entitat) throws I18NException {
        ...
        for (Enllaz e : enllazos) {
            filesToDelete.addAll(enllazLogicaEjb.deleteFullRecursive(e));
        }

        // Seccio BBDD + fitxers
        List<Seccio> seccions = seccioLogicaEjb.select(SeccioFields.ENTITATID.equal(entitat.getEntitatID()));
        for (Seccio s : seccions) {
            filesToDelete.addAll(seccioLogicaEjb.deleteFullRecursive(s));
        }
        ...
        filesToDelete.addAll(fitxersEntitat);

        // eliminar la entitat de BBDD
        delete(entitat);

        // obligam a realitzar les transaccions abans de eliminar els fitxers per evitar
        // errors de constraint #434
        getEntityManager().flush();

        // Borram els fitxers de BBDD
        fitxersEjb.delete(FitxerFields.FITXERID.in(fitxersEntitat));

        // eliminar els fitxers físics post COMMIT

        filesToDelete.removeAll(Collections.singleton(null));
