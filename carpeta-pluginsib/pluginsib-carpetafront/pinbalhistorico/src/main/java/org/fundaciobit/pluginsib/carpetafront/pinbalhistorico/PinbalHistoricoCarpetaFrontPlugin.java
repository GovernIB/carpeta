...
public class PinbalHistoricoCarpetaFrontPlugin extends AbstractPinbalCarpetaFrontPlugin {
    ...
    protected static final String HISTORIC_RES_BUNDLE = "carpetafrontpinbalhistorico";
	...
	public void serveiRestService(String absolutePluginRequestPath, String relativePluginRequestPath, String query,
		...
		DatosHistorico datosHistorico = cridadaRest(userData, codMunicipio, absolutePluginRequestPath, locale);
	...
	public DatosHistorico cridadaRest(UserData userData, String codMunicipio, String absolutePluginRequestPath, Locale locale) {
		...
		try {
			...
			SolicitudHistorico solicitud = new SolicitudHistorico(codigoProvincia, codMunicipio, tipoDoc,
					documentacion);
			...
			if (e.getMessage().contains("[0502]")) {
				datosHistorico.setError(
						getTraduccio(HISTORIC_RES_BUNDLE, "historicerror.1", locale));
			} else if(e.getMessage().contains("[0401]")){
				datosHistorico.setError(
						getTraduccio(HISTORIC_RES_BUNDLE, "historicerror.2", locale));
			}else {
		 ...
		 */
		/*
		 * datosHistorico.setError(""); datosHistorico.setDni("12345678A");
		 * datosHistorico.setNombre("JUAN ANTONIO");
		 * datosHistorico.setApellido1("GARCIA"); datosHistorico.setApellido2("");
		 * datosHistorico.setCodigo("0003"); datosHistorico.setDescripcion("Tramitada");
		 ...
		 * 
		 * Domicilio domicilio1 = new Domicilio(); domicilio1.setClaveHojaPadronal(new
		 * ArrayList<>(Arrays.asList(new ClaveHojaPadronal("distrito", "seccion",
		 * "hoja")))); domicilio1.setProvinciaRespuesta(new
		 * ArrayList<>(Arrays.asList(new ProvinciaRespuesta("07", "Baleares"))));
		 * domicilio1.setMunicipioRespuesta(new ArrayList<>(Arrays.asList(new
		 * MunicipioRespuesta("40", "Palma")))); domicilio1.setEntColectiva(new
		 * ArrayList<>(Arrays.asList(new EntColectiva("41", "EntColectiva"))));
		 * domicilio1.setEntSingular(new ArrayList<>(Arrays.asList(new EntSingular("42",
		 * "EntSingular")))); domicilio1.setNucleo(new ArrayList<>(Arrays.asList(new
		 * Nucleo("43", "Nucleo")))); domicilio1.setCodUnidadPoblacional("codUnidad");
		 * domicilio1.setDesde("01/01/1999"); domicilio1.setHasta("30/12/2005");
		 * domicilio1.setMotivoInscripcion(new ArrayList<>(Arrays.asList(new
		 * MotivoInscripcion("cod","mot","causa")))); domicilio1.setMotivoBaja(new
		 * ArrayList<>(Arrays.asList(new MotivoBaja("1","2","3"))));
		 * 
		 * Direccion direccion = new Direccion(); direccion.setVia(new
		 * ArrayList<>(Arrays.asList(new Via("cod", "tipo", "via"))));
		 * direccion.setKmt(""); direccion.setBloque(""); direccion.setPortal("B");
		 * direccion.setEscalera(""); direccion.setPlanta("6");
		 * direccion.setPuerta("A"); direccion.setCodigoPostal("07003");
		 * domicilio1.setDireccion(new ArrayList<>(Arrays.asList(direccion)));
		 * datosHistorico.setHistorico(new ArrayList<>(Arrays.asList(domicilio1,
		 * domicilio1)));
		 */
	...
	public static class SolicitudHistorico extends Solicitud {
		...
		public SolicitudHistorico(String provinciaSolicitud, String municipioSolicitud, String tipoDocumentacion,
		...
		public String getDatosEspecificos() { // xml
			 ...
			 * xmlBuilder.append(xmlOptionalStringParameter(this.numeroAnyos,
			 * "NumeroAnyos")); }
			 */
