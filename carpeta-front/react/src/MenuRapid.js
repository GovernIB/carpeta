...
class MenuRapid extends Component {
  ...
  render() {
    ...
    if (autenticat === "1") {
      if (this.state.error) {
        content = (
          <div className="alert alert-danger errorConnexio" role="alert">
            {t(this.state.error)}
          </div>
        );
      } else {
        this.state.items.forEach((s, i) => {
          ...
          switch (s.tipus) {
            case 0: // Plugin react
              allItems.push(
                <li key={"react" + i} className="nav-item pr-4 lletraRapid">
                  ...
                  >
                    <img
                      src={urlBase + s.urllogo}
                      alt={s.nom}
                      title={t("iconaDe") + " " + s.nom}
                      className="imc-icona"
                    />
                    {/*<img src={s.urllogo} alt="" title="" className="imc-icona"/>*/}
                    <span className="menuRapidView" id={"menuRapidBoto" + i}>
                      {s.nom}
                    </span>
                  </Link>
                </li>
            ...
            case 3: // Seccio
              ...
              s.plugins.forEach((p, j) => {
                if (p.reactComponent === true) {
                  ...
                  plugins.push(
                    ...
                    <Link className="dropdown-item navCarpeta linkVerd" key={"seccioReact"+j}
                      to={{
                        pathname: Constants.SECCIO_PATH + s.context + Constants.PLUGINREACT_PATH + p.context,
                        nomPagina: "plugin",
                      }}
                      tabIndex={301 + k + j + 1}
                      aria-labelledby={"menuRapidBoto" + i}
              ...
              // XYZ ZZZ  MACOSX nav-item pr-4 lletraRapid
              ...
              allItems.push(
                <li className="btn-group navCarpeta pr-4" key={"global" + i}>
                  <div className="dropdown">
                    <button
                      type="button"
                      className="btn btn-default dropdown-toggle p-0 lletraRapid disBlok text-verd"
                    ...
                    <div
                      className="dropdown-menu maxContent"
                      aria-labelledby={"menuRapidBoto" + i}
                      id={"submenu" + i}
                      style={newStyle}
                    >
        ...
        content = (
          <div>
            <nav className="navbar navbar-expand-sm bg-white p-0 fixo borderMenu">
              <ul
                className="navbar-nav mRapidGlobal"
                style={{ padding: "0.5rem!important", height: "auto" }}
                id="llistaMenuRapid"
              >
                {allItems}
              </ul>
    ...
    return <div id="menuRapid">{content}</div>;
  }
}
