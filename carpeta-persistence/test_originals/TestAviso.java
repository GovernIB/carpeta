...
public class TestAviso extends TestBase{
   ...
   public void testCreateAviso() {
      ...
      AvisJPA aviso = new AvisJPA();
      aviso.setDatafi(new Timestamp(System.currentTimeMillis()));
      aviso.setDatainici(new Timestamp(System.currentTimeMillis()));
      aviso.setTipus(0); //TipoAviso.INFO);

      //Obtenemos la entidad
      EntitatJPA entidad = obtenerEntidad("A04003007");
      aviso.setEntitat(entidad);
...
//      aviso.setTraducciones(traducciones);
      ...
      Assert.assertNotNull(aviso.getAvisID());
   }
   ...
   public void testObtenerAvisoByTipo() {
      ...
      TypedQuery<AvisJPA> query = em.createQuery(
         "select a from Aviso a where a.tipo = :tipo", AvisJPA.class);
      query.setParameter("tipo", 0);
      AvisJPA aviso = query.getSingleResult();

      // Comprovam el tipus de l'avis seleccionat
      Assert.assertEquals(0, aviso.getTipus());
