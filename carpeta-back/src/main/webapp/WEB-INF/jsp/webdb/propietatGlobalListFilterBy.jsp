...
<un:useConstants var="PropietatGlobalFields" className="es.caib.carpeta.model.fields.PropietatGlobalFields"/>
  ...
  <div id="FilterDiv" class="well formbox" style="${displayFilterDiv} margin-bottom:3px; margin-left: 1px; padding:3px;">
      ...
      <div class="input-prepend input-append" style="padding-right: 4px;padding-bottom: 4px;">
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PropietatGlobalFields.PROPIETAGLOBALID)}">
            <%-- FILTRE NUMERO --%>      
            <div class="input-prepend input-append" style="padding-right: 4px;padding-bottom: 4px;">
              <span class="add-on"><fmt:message key="propietatGlobal.propietaGlobalID" />:</span>

              <span class="add-on"><fmt:message key="genapp.from" /></span>
              
              <form:input cssClass="input-append input-small" path="propietaGlobalIDDesde" />


              <span class="add-on"><fmt:message key="genapp.to" /></span>

              <form:input cssClass="input-append input-small search-query" path="propietaGlobalIDFins" />

            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PropietatGlobalFields.CODI)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="propietatGlobal.codi" var="codi" />
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PropietatGlobalFields.VALUE)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="propietatGlobal.value" var="value" />
              <fmt:message key="genapp.form.searchby" var="cercapervalue" >                
                 <fmt:param value="${value}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${value}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercapervalue}" path="value" />
            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PropietatGlobalFields.DESCRIPCIO)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="propietatGlobal.descripcio" var="descripcio" />
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PropietatGlobalFields.ENTITATID)}">
            <%-- FILTRE NUMERO --%>      
            <div class="input-prepend input-append" style="padding-right: 4px;padding-bottom: 4px;">
              <span class="add-on"><fmt:message key="propietatGlobal.entitatID" />:</span>
