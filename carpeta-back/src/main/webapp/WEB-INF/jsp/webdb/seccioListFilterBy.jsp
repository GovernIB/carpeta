...
<un:useConstants var="SeccioFields" className="es.caib.carpeta.model.fields.SeccioFields"/>
  ...
  <div id="FilterDiv" class="wellgroupfilter formbox" style="${displayFilterDiv} margin-bottom:3px; margin-left: 1px; padding:3px;">
      ...
      <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,SeccioFields.SECCIOID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="seccio.seccioID" />:</span>

              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
              
              <form:input cssClass="input-append input-small" path="seccioIDDesde" />


              <span class="add-on">&nbsp;<fmt:message key="genapp.to" />&nbsp;</span>

              <form:input cssClass="input-append input-small search-query" path="seccioIDFins" />

            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,SeccioFields.NOMID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="seccio.nomID" />:</span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,SeccioFields.DESCRIPCIOID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="seccio.descripcioID" />:</span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,SeccioFields.CONTEXTE)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="seccio.contexte" var="contexte" />
              <fmt:message key="genapp.form.searchby" var="cercapercontexte" >                
                 <fmt:param value="${contexte}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${contexte}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercapercontexte}" path="contexte" />
            </div>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,SeccioFields.ACTIVA)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="seccio.activa" />:</span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,SeccioFields.SECCIOPAREID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
              <%-- FILTRE NUMERO SELECT MULTIPLE --%>
              <div class="input-group-prepend" style="padding-top: 5px;padding-right: 5px;">
                 <span class="add-on"><fmt:message key="seccio.seccioPareID" />:</span>
              </div>

              <div class="input-group-prepend" style="min-width:200px">
                <form:select id="seccio_seccioPareID_select" path="seccioPareIDSelect" cssClass="search-query input-medium form-control select2 select2-hidden-accessible" multiple="true" style="width:100%;" tabindex="-1" aria-hidden="true">
                    <c:forEach var="_entry" items="${__theFilterForm.mapOfValuesForSeccioPareID}">
                      <option value="${_entry.key}" ${fn:contains(__theFilterForm.seccioPareIDSelect, _entry.key)?'selected':''} >${_entry.value}</option>
                    </c:forEach>
                </form:select>
              </div>
              ...
              <script type="text/javascript">
                $(document).ready(function() {
                    $('#seccio_seccioPareID_select').select2({
                        closeOnSelect: false
                    });
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,SeccioFields.ENTITATID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="seccio.entitatID" />:</span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,SeccioFields.ORDRE)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="seccio.ordre" />:</span>
