 ...
 */
@Component
public class NotificacioAppRefList extends RefListBase
    implements NotificacioAppFields {

  @EJB(mappedName = NotificacioAppService.JNDI_NAME)
  private NotificacioAppService notificacioAppEjb;

  @EJB(mappedName = TraduccioService.JNDI_NAME)
  private TraduccioService traduccioEjb;
  public NotificacioAppRefList(NotificacioAppRefList __clone) {
    super(__clone);
    this.notificacioAppEjb = __clone.notificacioAppEjb;
    this.traduccioEjb = __clone.traduccioEjb;
  }
  public NotificacioAppRefList() {
    setSelects(new Select<?>[] { CODI.select });
    addCampTraduible(TITOLID.select);
    addCampTraduible(MISSATGEID.select);
  }
  public List<StringKeyValue> getReferenceList(Field<?> keyField, Where where, OrderBy ... orderBy) throws I18NException {
    Select<Long> _transSelect = checkTranslationFields();
    Select<StringKeyValue> select =  new org.fundaciobit.genapp.common.query.SelectMultipleStringKeyValue(keyField.select, getSeparator(), getSelects());
    List<StringKeyValue> list = notificacioAppEjb.executeQuery(select, where, (orderBy==null || orderBy.length == 0) ? getOrderBy() : orderBy);
    if (_transSelect == null) {
      return list;
    }
    // key => TransID | value => notificacioAppEjb_PK
    java.util.Map<String,String> keysMap = org.fundaciobit.genapp.common.utils.Utils.listToMapInverse(list);
    org.fundaciobit.genapp.common.query.Where _w1 = es.caib.carpeta.model.fields.TraduccioFields.TRADUCCIOID.in(notificacioAppEjb.executeQuery(_transSelect, where));
