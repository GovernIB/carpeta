...
public class AvisSuperAdminController extends AvisController {
    ...
    @Override
    public AvisForm getAvisForm(AvisJPA _jpa, boolean __isView, HttpServletRequest request,
    ...
    @Override
    public void postValidate(HttpServletRequest request, AvisForm avisForm, BindingResult result)
        ...
        if (__entidadid < 1L && __tipus != Constants.TIPUS_AVIS_BACK) {
            ...
            result.rejectValue(get(ENTITATID), "genapp.validation.required",
                    new String[] { I18NUtils.tradueix(get(ENTITATID)) }, null);
        }

        if (__pluginid < 1L && __tipus == Constants.TIPUS_AVIS_FRONT_PLUGIN) {
            // log.error("El camp Plugin és obligatori");
            result.rejectValue(get(PLUGINFRONTID), "genapp.validation.required",
                    new String[] { I18NUtils.tradueix(get(PLUGINFRONTID)) }, null);
        }

        if (__tipus == Constants.TIPUS_AVIS_BACK) {
            if (__entidadid > 0L) {
                // log.error("Els camp entitat no és necessari");
                result.rejectValue(get(ENTITATID), "avis.validation.tipusback",
                        new String[] { I18NUtils.tradueix(get(ENTITATID)) }, null);
            }
            if (__pluginid > 0L) {
                // log.error("Els camp plugin no és necessari");
                result.rejectValue(get(PLUGINFRONTID), "avis.validation.tipusback",
                        new String[] { I18NUtils.tradueix(get(PLUGINFRONTID)) }, null);
            }
        ...
        if (__tipus == Constants.TIPUS_AVIS_FRONT_PLUGIN) {
            if (__entidadid > 0L && __pluginid > 0L) {
                ...
                List<PluginEntitatJPA> pluginsEntitat = pluginEntitatEjb
                        .findAllByEntitatId(__entidadid);
                Boolean pertany = false;
                for (PluginEntitatJPA pluginEntitat : pluginsEntitat) {
                    pertany = (pluginEntitat.getPluginID() == __pluginid
                            && pluginEntitat.getEntitatID() == __entidadid);
                    if (pertany) {
                        break;
                    }
                }
                if (!pertany) {
                    ...
                    result.rejectValue(get(PLUGINFRONTID), "avis.validation.pluginentitat",
