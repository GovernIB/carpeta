...
public class EstadisticaSuperAdminController extends EstadisticaController {
    ...
    public EstadisticaFilterForm getEstadisticaFilterForm(Integer pagina, ModelAndView mav,
        ...
        if (estadisticaFilterForm.isNou()) {
            ...
            estadisticaFilterForm.addGroupByField(DATAESTADISTICA);
    ...
    public List<StringKeyValue> getReferenceListForEntitatID(HttpServletRequest request,
        ...
        List<StringKeyValue> entitats = entitatRefList.getReferenceList(EntitatFields.ENTITATID,
                where);
        ...
        if (!entitatIdKeys.contains(estadisticaForm.getEstadistica().getEntitatID().toString())) {
            entitats.add(
                    new StringKeyValue(estadisticaForm.getEstadistica().getEntitatID().toString(),
    ...
    @Override
    public List<StringKeyValue> getReferenceListForEntitatID(HttpServletRequest request,
            ModelAndView mav, EstadisticaFilterForm estadisticaFilterForm, List<Estadistica> list,
        ...
        for (Estadistica estadistica : list) {
            if (estadistica.getEntitatID() != null) {
                if (!entitatIdKeys.contains(estadistica.getEntitatID().toString())) {
                    entitats.add(new StringKeyValue(estadistica.getEntitatID().toString(),
                            I18NUtils.tradueix("entitat.esborrada")));
                }
            }
        }
